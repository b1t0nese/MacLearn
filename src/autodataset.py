...
class AutoDataset(QObject):
    ...
    def run(self):
        ...
        for class_data in self.project_data["classes"]:
            size = tuple(
                map(int, self.project_data["configuration"]["images_size"].split("x"))
            )
            for subclass_data in class_data["subclasses"]:
                if class_data["enabled"] and self._is_running:
                    img_counts = self.project_data["configuration"][
                        "images_per_class"
                    ] // len(class_data["subclasses"])
                    self.download_images(
                        subclass_data, img_counts, class_data["class_id"], size=size
                    )
